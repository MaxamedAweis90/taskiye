...
export const OfflineEmptyState: React.FC<OfflineEmptyStateProps> = ({
  ...
  const { retryConnection, isChecking: globalChecking } = useOnlineStatus();
    ...
    return (
      <div className="flex flex-col items-center justify-center p-4 text-center rounded-2xl bg-slate-50 dark:bg-white/[0.03] border border-slate-200/80 dark:border-white/[0.06] my-2 select-none">
        ...
        <button
          ...
          disabled={isChecking}
          onClick={handleCheck}
          className="mt-2.5 px-3 py-1 rounded-lg text-xs font-semibold bg-amber-500 hover:bg-amber-600 disabled:opacity-60 text-white shadow-sm transition-all cursor-pointer flex items-center gap-1 active:scale-95"
        >
          {isChecking ? (
            <>
  ...
  return (
    <div className="w-full min-h-[60vh] flex flex-col items-center justify-center py-16 px-4 text-center rounded-3xl bg-white/60 dark:bg-[#10192D]/60 border border-slate-200/80 dark:border-white/[0.08] shadow-sm my-auto select-none">
      ...
      <button
        ...
        disabled={isChecking}
        onClick={handleCheck}
        className="mt-5 px-4 py-2.5 rounded-xl text-xs font-bold bg-amber-500 hover:bg-amber-600 disabled:opacity-60 text-white shadow-md shadow-amber-500/20 transition-all cursor-pointer flex items-center gap-1.5 active:scale-95"
      >
        {isChecking ? (
          <>
