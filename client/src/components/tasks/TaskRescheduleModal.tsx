...
export const TaskRescheduleModal: React.FC<TaskRescheduleModalProps> = ({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-md bg-[#0F172A] border border-amber-400/30 rounded-3xl p-5 sm:p-6 shadow-[0_20px_50px_rgba(0,0,0,0.9),0_0_30px_rgba(250,204,21,0.15)] flex flex-col gap-4 animate-in zoom-in-95 duration-150"
      ...
      >
        ...
        {/* Task Preview Card */}
        <div className="p-3 rounded-xl bg-[#152033] border border-white/[0.06] flex items-start gap-2">
          <Sparkles className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" />
          <p className="text-xs font-semibold text-slate-200 line-clamp-2 leading-relaxed break-words">
            "{taskTitle}"
          </p>
        </div>
            ...
            {selectedOption === 'custom' && (
              <div className="pt-2 border-t border-white/[0.08]" onClick={(e) => e.stopPropagation()}>
                <input
                  ...
                  className="w-full bg-[#0F172A] border border-amber-400/50 rounded-xl px-3 py-2 text-xs font-semibold text-amber-300 focus:outline-none focus:ring-1 focus:ring-amber-400 cursor-pointer"
                />
              </div>
            )}
          </div>
